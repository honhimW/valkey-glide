...
fn main() {
    ...
    let java_cfg = JavaConfig::new(
        Path::new("client").join("src").join("main")
            .join("java").join("glide").join("ffi").join("callback"),
        "glide.ffi.callback".into(),
    )
    ...
    let in_src = Path::new("src").join("async_callback.rs.in");
    let out_src = Path::new(&out_dir).join("async_callback.rs");
